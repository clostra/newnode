...
function do_curl {
    local proxy=localhost:$1
    local host=$2
    local code=$(curl -x $proxy $host -o /dev/null -w "%{http_code}" --silent --show-error)
    element_in $code "${@:3}" && return 0 || return 1
}
...
if [ "$r" == "0" ]; then
    ...
    if ! do_curl $proxy_tcp_port https://google.com $HTTP_OK $HTTP_FOUND $HTTP_MOVED; then
        r=2
    fi
fi
