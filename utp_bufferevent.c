...
void ubev_utp_closed(utp_bufferevent *u)
{
    u->utp = NULL;
    if (u->other_bev) {
...
uint64 utp_on_state_change(utp_callback_arguments *a)
{
    ...
    if (a->state != UTP_STATE_WRITABLE) {
        //debug("%s %p state:%d %s\n", __func__, a->socket, a->state, utp_state_names[a->state]);
    }
    if (!u) {
        return 0;
    }
...
void ubev_event_cb(bufferevent *bev, short events, void *ctx)
{
    ...
    if (events & BEV_EVENT_EOF) {
        // XXX: this is technically wrong. if no writing error has occurred we can't tell this from a shutdown
        u->bev_eof = true;
        if (!evbuffer_get_length(bufferevent_get_output(u->bev))) {
            bufferevent_disable(u->bev, EV_WRITE);
        } else {
            debug("%s evhttp directed buffer not empty len:%zu\n", __func__, evbuffer_get_length(bufferevent_get_output(u->bev)));
        }
    }
